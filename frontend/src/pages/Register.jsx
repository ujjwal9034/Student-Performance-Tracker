...
const Register = () => {
  ...
  const textFieldSx = {
    ...
    '& .MuiSelect-icon': { color: isLight ? '#64748b' : 'rgba(255,255,255,0.55)' }
  };
      ...
      <div className={`hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-between p-12 ${isLight ? 'bg-gradient-to-br from-indigo-50 via-white to-purple-50' : 'bg-slate-900'}`}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full max-w-lg mx-auto text-center">
          ...
          <Typography variant="h2" fontWeight="900" sx={{ mb: 2, lineHeight: 1.1, color: isLight ? '#0f172a' : 'white', letterSpacing: '-0.02em' }}>
            Join the <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-500">Platform</span>
          </Typography>
          <Typography variant="body1" sx={{ opacity: 0.9, lineHeight: 1.7, fontSize: '1.05rem', color: isLight ? '#475569' : '#cbd5e1' }}>
            Start tracking your academic journey today and take control of your educational progress.
          </Typography>
        </div>
      </div>
      ...
      <div className={`w-full lg:w-1/2 flex items-center justify-center p-8 relative ${isLight ? 'bg-gray-50' : 'bg-slate-950'}`}>
        ...
        >
          <div className="mb-8 text-center">
            <Typography variant="h4" fontWeight="800" sx={{ mb: 1, color: isLight ? '#4f46e5' : '#818cf8', letterSpacing: '-0.01em' }}>Create Account</Typography>
            <Typography variant="body2" sx={{ color: isLight ? '#64748b' : '#94a3b8' }}>Request access to the portal</Typography>
          </div>
          ...
          {success ? (
            <div className={`text-center p-6 rounded-xl ${isLight ? 'bg-emerald-50 border border-emerald-200' : 'bg-emerald-500/10 border border-emerald-500/25'}`}>
              <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 1.5, color: isLight ? '#059669' : '#34d399' }}>Registration Received!</Typography>
              <Typography variant="body2" sx={{ mb: 3, lineHeight: 1.8, color: isLight ? '#475569' : '#cbd5e1' }}>
                Your account is currently <strong>pending admin approval</strong>. 
                You will be notified once your teacher or administrator approves your account and grants you access.
              </Typography>
              <Button 
                variant="outlined" 
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4.5">
              <TextField fullWidth name="name" label="Full Name" value={formData.name} onChange={handleChange} required variant="outlined" sx={textFieldSx} />
              <TextField fullWidth type="email" name="email" label="Email Address" value={formData.email} onChange={handleChange} required variant="outlined" sx={textFieldSx} />
              <TextField fullWidth type="password" name="password" label="Password" value={formData.password} onChange={handleChange} required variant="outlined" sx={textFieldSx} />
              ...
              <TextField 
                ...
                sx={textFieldSx}
              >
                <MenuItem value="student" sx={menuItemSx}>Student</MenuItem>
                <MenuItem value="teacher" sx={menuItemSx}>Teacher</MenuItem>
              </TextField>

              {formData.role === "student" && (
                <TextField fullWidth type="number" name="semester" label="Semester" value={formData.semester} onChange={handleChange} required variant="outlined" sx={textFieldSx} />
              )}
          ...
          {!success && (
            <Box mt={4} textAlign="center">
              <Typography variant="body2" sx={{ color: isLight ? '#64748b' : '#94a3b8' }}>
                ...
                <Link to="/login" className="text-[#818cf8] font-medium hover:underline hover:text-indigo-300 transition-colors">Log in</Link>
              </Typography>
            </Box>
          )}
        </Paper>
